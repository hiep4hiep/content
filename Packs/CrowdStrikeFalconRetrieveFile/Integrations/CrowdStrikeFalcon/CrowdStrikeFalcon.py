...
''' CUSTOM COMMANDS '''
# New command
def get_extracted_file_command_base64(args):
    host_id = args.get('host_id')
    sha256 = args.get('sha256')
    filename = args.get('filename')
...
# New command
def rtr_polling_retrieve_file_base64_command(args: dict):
    """
    This function is generically handling the polling flow.
    In this case, the polling flow is:
    1. run the "cs-falcon-run-get-command" command to get the request id.
    ...
    if not args.get('SHA256'):
        ...
        if args.get('SHA256'):
            # the status is ready, we can get the extracted files
            args.pop('SHA256')
            return rtr_get_extracted_file_base64(get_status_response, args.get('filename'))  # type:ignore
...
# New command
def rtr_get_extracted_file_base64(args_to_get_files: dict, file_name: str):
    files = []
    outputs_data = []

    for host_id, values in args_to_get_files.items():
        arg = {'host_id': host_id, 'sha256': values.get('SHA256'), 'filename': file_name}
        file = get_extracted_file_command_base64(arg)
        files.append(file)
        outputs_data.append(
            {'HostID': arg.get('host_id'),
             'FileData': file
    ...
    return [CommandResults(readable_output="CrowdStrike Falcon files", outputs=outputs_data,
                           outputs_prefix="CrowdStrike.File")]
...
def main():
    ...
    try:
        ...
        elif command == 'cs-falcon-rtr-retrieve-file-base64':
            return_results(rtr_polling_retrieve_file_base64_command(args))
        else:
