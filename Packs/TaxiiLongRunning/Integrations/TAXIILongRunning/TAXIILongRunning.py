...
def module_test_command(client, limit, fetch_full_feed):
    ...
    initial_interval, _ = parse_date_range(
            demisto.params().get("initial_interval"), date_format=TAXII_TIME_FORMAT
        )
    ...
    try:
        ...
        if demisto.command() == 'long-running-execution':
            while True:
                try:
                    limit = -1
                    last_run_indicators = demisto.getIntegrationContext()
                    ...
                    set_integration_context(last_run_indicators)

                    demisto.debug("######## FETCH OK ########")
                    demisto.updateModuleHealth("Fetched " + str(len(indicators)))
                except Exception as e:
                    demisto.debug("######## FAILED ########" + str(e))
                    demisto.updateModuleHealth(str(e))
                finally:
                    demisto.debug("######## Finish fetch loop ########")
            ...
            demisto.updateModuleHealth('Fetched')
        else:
        ...
        for envelope in envelopes:
            ...
            for obj in stix_objects:
                ...
                if result := parse_objects_func[obj_type](obj):
                    ...
                    self.update_last_modified_indicator_date(obj.get(TIMESTAMP_FIELD))
