...
def generate_parse_xql(parent_field_name, parent_field_data):
    """
    args:
    ...
    if isinstance(parent_field_data,list):
        """
        for sub_list_field in parent_field_data[0]:
            if isinstance(parent_field_data[0][sub_list_field],str):
                query += f'{parent_field_name}_{sub_list_field} = arrayindex(json_extract_array({parent_field_name},"$."),0) -> {sub_list_field},\n'
            if isinstance(parent_field_data[0][sub_list_field],int):
                query += f'{parent_field_name}_{sub_list_field} = arrayindex(json_extract_array({parent_field_name},"$."),0) -> {sub_list_field},\n'
            elif isinstance(parent_field_data[0][sub_list_field],list):
                query += f'{parent_field_name}_{sub_list_field} = arrayindex(json_extract_array({parent_field_name},"$."),0) -> {sub_list_field}[],\n'
            elif isinstance(parent_field_data[0][sub_list_field],dict):
                query += f'{parent_field_name}_{sub_list_field} = arrayindex(json_extract_array({parent_field_name},"$."),0) -> {sub_list_field}' + '{},\n'
                nested_field_name = f'{parent_field_name}_{sub_list_field}'
                nested_field_data = parent_field_data[0][sub_list_field]
                query += generate_parse_xql(nested_field_name, nested_field_data)
        """     
        for item in parent_field_data:
            for sub_list_field in item:
                if isinstance(item[sub_list_field],str):
                    query += f'| alter {parent_field_name}_{sub_list_field} = arrayindex(json_extract_array({parent_field_name},"$."),0) -> {sub_list_field}\n'
            ...
            for j in range(0,len(row)):
                ...
                if isinstance(row[j],str):
                    if "{" in row[j] and "}" in row[j]:
                        ...
                        #except:
                        #    print("FAILED: " + title[j] + row[j])
    full_query_set = set(full_query.split("\n"))
    #return "  " + "\n  ".join(sorted(list(filter(None,full_query_set)))) + "\n;" # Remove '' and create parsing rules format
    return "\n ".join(full_query_set) # Remove '' and create parsing rules format                        
