...
# Get home directory
default_target_directory="/home/ec2-user"

# Check if the argument exists, if not, set the default target directory
if [ -z "$1" ]; then
    target_directory="$default_target_directory"
    echo "Target directory is not provided, using the default path /home/ec2-user"
else
    target_directory="$1"
fi
...
chmod +x cortex*.sh
./cortex-8.4.0.123787.sh --target "$target_directory"
...
./collector-1.4.1.1089.sh --target "$target_directory"