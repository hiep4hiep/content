...
def search_sentence_in_faiss(sentence):
    ...
    index = faiss.read_index("rawlog_faiss.index")
    with open("rawlog_data_mapping.pkl", "rb") as f:
        data_models = pickle.load(f)
    with open("rawlog_data_mapping_raw_log.pkl", "rb") as f:
        sample_log = pickle.load(f)
    ...
    for idx in indices[0]:
        return data_models[idx], sample_log[idx]
